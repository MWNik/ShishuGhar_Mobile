...
class BlockDataHelper {
  ...
  Future<List<TabBlock>> getTabBlockList() async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.query('tabBlock',orderBy: 'value ASC');
 ...
 Future<List<TabBlock>> getBlockListByBlockId(List<int>blockId) async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery('Select * from tabBlock where name in (${blockId.join(', ')}) order By value ASC');
  ...
  Future<void> insertMasterBlock(
      List<TabBlock> blockList) async {
