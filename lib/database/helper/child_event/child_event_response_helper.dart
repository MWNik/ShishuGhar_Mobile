...
class ChildEventTabResponceHelper {
  ...
  Future<List<ChildEventTabResponceModel>> childEventsByChild(
    ...
    var query = 'Select * from  child_event_responce  where creche_id=? and childenrolledguid=? ORDER BY CASE  WHEN update_at IS NOT NULL AND length(RTRIM(LTRIM(update_at))) > 0 THEN update_at ELSE created_at END DESC';
  ...
  Future<List<ChildEventTabResponceModel>> getEditedChildEventsForUpload() async {
