...
class ChildEventMetaFieldsHelper {
  DatabaseHelper databaseHelper = DatabaseHelper();
  Future<void> insertChildEventMeta(
    ...
    if (houseFieldItem.isNotEmpty) {
      await DatabaseHelper.database!.transaction((txn) async{
        for(var element in houseFieldItem){
          await txn.insert('tab_child_event',element.toJson());
        }
      });
      // for (var element in houseFieldItem) {
      //   await DatabaseHelper.database!
      //       .insert('tab_child_event', element.toJson());
      // }
    }
  }
  ...
  Future<List<HouseHoldFielItemdModel>> getChildEventMetaFields() async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tab_child_event where  hidden=0 ORDER by idx asc');
  ...
  Future<List<HouseHoldFielItemdModel>> getChildEventMetaFieldsbyScreenType(
    ...
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tab_child_event where  parent=? and hidden=0 ORDER by idx asc',
        [screen_type]);
