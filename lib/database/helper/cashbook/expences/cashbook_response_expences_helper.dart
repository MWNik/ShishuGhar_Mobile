...
class CashBookResponseExpencesHelper {
  ...
  Future<List<CashbookExpencesResponseModel>> getCashbookExpencesResponcewithGuid(
    String ceGuid,
  ) async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tab_cashbook_expences_response where cashbook_guid=?', [ceGuid]);
  ...
  Future<List<CashbookExpencesResponseModel>> cashbookExpencesByCreche(
    ...
    var query = 'Select * from  tab_cashbook_expences_response  where creche_id=? ORDER BY CASE  WHEN update_at IS NOT NULL AND length(RTRIM(LTRIM(update_at))) > 0 THEN update_at ELSE created_at END DESC';
  ...
  Future<List<CashbookExpencesResponseModel>> getEditedCashBookForExpenceUpload() async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!
        .rawQuery('select * from tab_cashbook_expences_response where is_edited=1');
    List<CashbookExpencesResponseModel> items = [];
