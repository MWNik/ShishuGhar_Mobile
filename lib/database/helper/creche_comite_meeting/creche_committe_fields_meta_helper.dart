...
class CrecheCommitteFieldsMetaHelper {
  ...
  Future<List<HouseHoldFielItemdModel>> getCrecheCommitteMetaFields() async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tab_creche_committe where  hidden=0 ORDER by idx asc');
  ...
  Future<List<HouseHoldFielItemdModel>> callMultiSelectTabItem() async {
    String query =
        'select * from tab_creche_committe where parent in (select options from tab_creche_committe where fieldtype =?)';
  ...
  Future<List<HouseHoldFielItemdModel>> getCrecheCommitteMetaFieldsbyScreenType(
      String screen_type) async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tab_creche_committe where  parent=? and fieldname != ? and hidden=0 ORDER by idx asc',
        [screen_type, 'status']);
