...
class CrecheDataHelper {
  Future<void> inserts(CresheDatabaseResponceModel items) async {
    await DatabaseHelper.database!.insert(
        'tab_creche_response', items.toJson(),
  ...
  Future<void> downloadCrecheData(Map<String, dynamic> item) async {
    ...
    hhData.forEach((element) async {
      ...
      if(hhData!=null){
        List<Map<String, dynamic>> careGiver = List<Map<String, dynamic>>.from(hhData['creche_caregiver_table']);
  ...
  Future<void> updateDownloadeData(Map<String, dynamic> item) async {
   var hhData=item['data'];
      if(hhData!=null){
        ...
        var childImage = await ImageFileTabHelper().getImageByDoctypeIdAndImbeNotNull(name.toString(), CustomText.Creches);
  ...
  Future<List<Map<String, dynamic>>> callCrechEnrolledByChildGUID(String ChildEnrollGUID) async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select ecr.*,cr.responces as crResponces from enrollred_exit_child_responce ecr left join tab_creche_response as cr on ecr.creche_id=cr.name where ecr.ChildEnrollGUID=? and ecr.date_of_exit isnull',[ChildEnrollGUID]);
  ...
  Future<List<CresheDatabaseResponceModel>> getCrecheResponceItem(int name) async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tab_creche_response where name=?',[name]);
    List<CresheDatabaseResponceModel> items = [];
