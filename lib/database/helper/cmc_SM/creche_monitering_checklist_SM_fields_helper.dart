...
class CrecheMoniteringCheckListSMFieldsHelper {
  DatabaseHelper databaseHelper = DatabaseHelper();

  Future<void> insertcmcSMMeta(
      List<HouseHoldFielItemdModel> houseFieldItem) async {
    if (houseFieldItem.isNotEmpty) {
      await DatabaseHelper.database!.transaction((txn) async{
        for(var element in houseFieldItem) {
          await txn.insert('tabCreche_Monitering_CheckList_SM', element.toJson());
        }
      });
    }
  }
  ...
  Future<List<HouseHoldFielItemdModel>> getcmcCCMetaFields() async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tabCreche_Monitering_CheckList_SM where  hidden=0 ORDER by idx asc');
  ...
  Future<List<HouseHoldFielItemdModel>> callMultiSelectTabItem() async {
    String query =
        'select * from tabCreche_Monitering_CheckList_SM where parent in (select options from tabCreche_Monitering_CheckList_SM where fieldtype =?)';
  ...
  Future<List<HouseHoldFielItemdModel>> getcmcCCMetaFieldsbyScreenType(
    ...
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.rawQuery(
        'select * from tabCreche_Monitering_CheckList_SM where  parent=? and fieldname != ? and hidden=0 ORDER by idx asc',
        [screen_type, 'status']);
