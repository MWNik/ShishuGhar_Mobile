...
class StateDataHelper {
  ...
  Future<List<TabState>> getTabNativeStateList() async {
    List<Map<String, dynamic>> result = await DatabaseHelper.database!.query('tabNativState',orderBy: 'value ASC');
  ...
  Future<void> insertMasterNativeStates(
      List<TabState> stateList)
  async {
    databaseHelper.openDb();
    await DatabaseHelper.database!.delete('tabNativState');
    if (stateList.isNotEmpty) {
      try {
        await DatabaseHelper.database!.transaction((txn) async {
          for(var element in stateList) {
            await txn.insert('tabNativState', element.toJson());
          }
        });
      } on Exception catch (e) {
        // TODO
