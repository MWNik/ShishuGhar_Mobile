...
class _CheckInsScreenState extends State<CheckIns> {
  ...
  Widget build(BuildContext context) {
    return
      WillPopScope(
        ...
        child: Scaffold(
        appBar: CustomAppbar(
          text: lng!=null?'${Global.returnTrLable(translatsLabel, CustomText.checkInAs, lng!)} $crecheName':CustomText.checkIN,
          onTap: () {
            Navigator.pop(context, 'itemRefresh');
          },
        ),
        floatingActionButton:
        currentDateAttendece?null:InkWell(
          onTap: () async {
            String hhGuid = '';
            if (!Global.validString(hhGuid)) {
              ...
              var refStatus = await Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (BuildContext context) => CheckInDetailsScreen(
                      ccinguid:hhGuid,
                      isEdit:false,
                ...
                (items.length > 0)?ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return GestureDetector(
                         onTap: () async {
                           var lstDate=await callDatesAlredDateList(Global.getItemValues(items[index].responces, 'date_of_checkin'));
                           var refStatus = await Navigator.of(context).push(
                             MaterialPageRoute(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Column(
                                    ...
                                    children: [
                                      Text(Global.returnTrLable(translatsLabel, CustomText.dateCheckin, lng!)
                                       ,
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      MainAxisAlignment.start,
                                      children: [
                                        Text(Global.validString(Global.getItemValues(items[index].responces, 'date_of_checkin'))
                                            ?Validate().displeDateFormate(Global.getItemValues(items[index].responces, 'date_of_checkin')):'',
                                          style: Styles.blue125,
                                          strutStyle: StrutStyle(height: 1),overflow:TextOverflow.ellipsis
                                        ),
                                        Text(Global.getItemValues(items[index].responces, 'checkin_location'),
                                          style: Styles.blue125,
                                          strutStyle: StrutStyle(height: 1),overflow:TextOverflow.ellipsis
                                        ),
                                      ],
  ...
  initData() async {
    ...
    var crechItemData = await CrecheDataHelper().getCrecheResponceItem(widget.crechId);
    if (crechItemData.length > 0) {
      crecheName=Global.getItemValues(crechItemData[0].responces!, 'creche_name');
    }
    ...
    items =  await CheckInResponseHelper().getCheckinsItem(widget.crechId.toString());
