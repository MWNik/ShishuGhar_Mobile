...
import '../../../../database/helper/cashbook/receipt/cashbook_receipt_meta_fileds_heler.dart';
...
class CashBookReceiptDetailsScreen extends StatefulWidget {
  final String creche_id;
  final int name;

  CashBookReceiptDetailsScreen({
    super.key,
  ...
  @override
  State<CashBookReceiptDetailsScreen> createState() =>
      _CashBookReceiptDetailsScreenState();
}

class _CashBookReceiptDetailsScreenState
    extends State<CashBookReceiptDetailsScreen> {
  ...
  Future<void> initializedData() async {
    ...
    await callScrenControllers('Cashbook Receipt');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        appBar: CustomAppbar(
          text: Global.returnTrLable(translats, CustomText.CashBookReceipt, lng!),
  ...
  Future<void> callScrenControllers(screen_type) async {
    ...
    lng = (await Validate().readString(Validate.sLanguage))!;
    await CashbookRceiptMetaFieldsHelper()
  ...
  Future<void> saveDataInData() async {
    if (allItems.length > 0) {
      ...
      var eventItem = CashbookReceiptResponseModel(
          name: myMap['name'],
      ...
      await CashBookReceiptResponseHelper().inserts(eventItem);
    }
  }
