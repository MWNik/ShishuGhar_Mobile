...
class CashBookListingTabScreen extends StatefulWidget {
  final String creche_id;

  CashBookListingTabScreen({super.key, required this.creche_id});

  @override
  State<CashBookListingTabScreen> createState() =>
      _CashBookListingTabScreenState();
}

class _CashBookListingTabScreenState extends State<CashBookListingTabScreen>
  ...
  List<Widget> tabControllerScreen() {
    ...
    for (int i = 0; i < tabCount; i++) {
      if (i == 1) {
        tabItem.add(CashBookReceiptListingScreen(
          creche_id: widget.creche_id,
        ));
      } else
        tabItem.add(CashBookExpensesListingScreen(creche_id: widget.creche_id));
    }
    return tabItem;
  }
