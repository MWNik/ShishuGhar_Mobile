...
class ExitedChildDetailView extends StatefulWidget {
  ...
  @override
  _ExitedChildDetailViewState createState() => _ExitedChildDetailViewState();
}

class _ExitedChildDetailViewState extends State<ExitedChildDetailView> {
      ...
      case 'Date':
        return CustomDatepickerDynamic(
          ...
          isRequred: quesItem.reqd == 1
              ? quesItem.reqd
              : logic!.dependeOnMendotory(myMap, quesItem),
          isVisible: quesItem.fieldname == 'date_of_enrollment'
              ? true
  ...
  Future<void> callScrenControllers(screen_type) async {
      ...
      if (Global.validString(allItems[i].options)) {
        if (allItems[i].options == 'Creche') {
          await OptionsModelHelper()
              .callCrechInOption(
                  allItems[i].options!, Global.stringToInt(widget.creche_id))
    ...
    await FormLogicDataHelper().callFormLogic(screen_type).then((data) {
      logic = DependingLogic(labelControlls, logics, lng);
    });
    setState(() {
      _isLoading = false;
    });
  ...
  Future<void> updateHiddenValue() async {
    ...
    if (alrecords.length > 0) {
      ...
      // var creCheResponce = await CrecheDataHelper()
      //     .getCrecheResponceItem(Global.stringToInt(widget.creche_id));
      // if(creCheResponce.length>0){
      //   myMap['creche_name']=Global.getItemValues(creCheResponce.first.responces, 'creche_name');
      // }

      // var childEnrolledResponce = await EnrolledChilrenResponceHelper()
      //     .callChildrenResponce(alrecords.first.childenrolledguid!);

      // if(childEnrolledResponce.length>0){
      //   // myMap['date_of_enrollment']=Global.getItemValues(childEnrolledResponce.first.responces, 'date_of_enrollment');
      //   myMap['age_at_enrollment_in_months']=Global.stringToInt(Global.getItemValues(childEnrolledResponce.first.responces, 'age_at_enrollment_in_months'));
      // }
    }
  }
