...
class ChildReferralTabItemsViewScreen extends StatefulWidget {
  ...
  @override
  State<ChildReferralTabItemsViewScreen> createState() =>
      _ChildFollowUpTabItemSCreenState();
}

class _ChildFollowUpTabItemSCreenState
    extends State<ChildReferralTabItemsViewScreen> {
    ...
    switch (quesItem.fieldtype) {
      ...
      case 'Float':
        return DynamicCustomTextFieldFloat(
          ...
          readable: quesItem.fieldname == 'weight_on_discharge'?(isWeightOnDischargeeditable == true?false:true):true,
          isVisible:
              DependingLogic().callDependingLogic(logics, myMap, quesItem),
          onChanged: (value) {
            print('Entered text: $value');
  ...
  Future<void> saveDataInData() async {
    ...
    if (widgets != null) {
      ...
      var itemz = ChildReferralTabResponceModel(
          child_referral_guid: widget.child_referral_guid,
          childenrolledguid: widget.enrolChildGuid,
          cgmguid: widget.GrowthMonitoringGUID,
          date_of_referral: myMap['date_of_referral'],
          visit_count:  myMap['visit_count'],
          schedule_date:  myMap['schedule_date'],
          name: name as int?,
      ...
      await ChildReferralTabResponseHelper().inserts(itemz);

      // if (!reffrralDateRedable) {
      //   await ChildFollowUpTabResponseHelper().deleteFollowUpBYReffreral(
  ...
  Future<void> updateHiddenValue() async {
    ...
    if (alredRecord.isNotEmpty) {
      ...
      if(date.add(Duration(days: 31)).isAfter(DateTime.parse(Validate().currentDate()))){
