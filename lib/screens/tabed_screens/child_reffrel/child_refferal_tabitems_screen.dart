...
class _ChildFollowUpTabItemSCreenState
  ...
  Future<void> initializeData() async {
    ...
    multselectItemTab=await ChildReferralFieldsHelper().callMultiSelectTabItem();
  ...
  List<Widget> cWidget(
    ...
    if (items != null) {
      for (int i = 0; i < items.length; i++) {
        ...
        }else  screenItems.add(SizedBox(height: 5.h));
      }
    }
    return screenItems;
  }
    ...
    switch (quesItem.fieldtype) {
      case 'Link':
        ...
        return DynamicCustomDropdownField(
          ...
          readable: quesItem.fieldname=='child_status'?widget.isDischarge:null,
          items: items,
      ...
      case 'Table MultiSelect':// Multi select Drop Down
        ...
        List<OptionsModel> items = options
            .where((element) => element.flag == 'tab${quesItem.multiselectlink}')
            .toList();
        ...
        return DynamicMultiCheckGridView(
          items: items,
          isRequred: quesItem.reqd==1?quesItem.reqd:DependingLogic().dependeOnMendotory(logics, myMap, quesItem),
          titleText:  Global.returnTrLable(translats, quesItem.label!.trim(), lng),
          selectedItem: myMap[quesItem.fieldname],
          isVisible:
          DependingLogic().callDependingLogic(logics, myMap, quesItem),
        ...
        return CustomDatepickerDynamic(
          ...
          isRequred: quesItem.reqd==1?quesItem.reqd:DependingLogic().dependeOnMendotory(logics, myMap, quesItem),
          readable: (quesItem.fieldname == 'date_of_referral'||quesItem.fieldname == 'discharge_date'||quesItem.fieldname == 'visit_date'||quesItem.fieldname == 'admission_date'
              ? widget.isDischarge
              : false),
          calenderValidate: DependingLogic().calenderValidation(logics, myMap, quesItem),
          onChanged: (value) {
            // if(quesItem.fieldname == 'visit_date'){
            //   if(value != null){
            //     myMap.remove('admission_date');
            //     setState(() {});
            //   }
            // }
            myMap[quesItem.fieldname!] = value;
  ...
  Future<void> saveDataInData(int visitCount) async {
    ...
    if (widgets != null) {
      ...
      await ChildReferralTabResponseHelper().insertUpdate(
          ...
          myMap['schedule_date'],
          responcesJs,
          userName,widget.GrowthMonitoringGUID);
  ...
  callAutoGeneratedOtherTable(Map<String, dynamic> answred,HouseHoldFielItemdModel parentItem ){
    var parentQlogic=logics.where((element) =>
    element.dependentControls==parentItem.fieldname &&(element.type_of_logic_id=='21')
    ).toList();
            ...
            if(algoData[2]=='child_dob'){
              ...
              if(dependValue!=null&&widget.ChildDOB!=null) {
                ...
                if(algoData[1].toLowerCase()=='m')
                  calucalteDate=Validate().calculateAgeInMonthsDepenExp(dateDob,dependDate);
                else  if(algoData[1].toLowerCase()=='y')
                  calucalteDate=Validate().calculateAgeInYearDepenExp(dateDob,dependDate);
                else  if(algoData[1].toLowerCase()=='d')
                  calucalteDate=Validate().calculateAgeInDaysEx(dateDob,dependDate);
  ...
  DateTime callRequredMinimumDate(String fieldname){
    DateTime minimumDate=widget.minDate;
    if (fieldname == 'visit_date') {
      if(Global.validString(myMap['date_of_referral'])){
        List<int> parts = myMap['date_of_referral'].toString().split('-').map(int.parse).toList();
          ...
          if (Global.stringToInt(
            ...
            visitCount= 3;
              ...
              Global.stringToDouble(
                  growhthDetails['weight_for_height'].toString()) ==
                  2) {
            DateTime folloupDate = followUpGenratedDate!.add(Duration(days: 15));
            await ChildFollowUpTabResponseHelper().autoCreateFollowRecord(
                '${DateFormat('yyyy-MM-dd').format(folloupDate)}',
                widget.child_referral_guid,
                widget.enrolChildGuid,
