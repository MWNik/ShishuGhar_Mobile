...
class _AttendanceListState extends State<AttendanceListedScreen> {
  ...
  List<ChildAttendanceResponceModel> unsynchedList = [];
  List<ChildAttendanceResponceModel> allList = [];
  ...
  Future<void> attendeceChildRecord() async {
    ...
    unsynchedList = childAttendance
        ...
        .toList();
    allList = childAttendance;
    FilteredAttendance = isOnlyUnsynched ? unsynchedList : allList;
  ...
  filterDataQu(String entry) {
    if (entry.length > 0) {
      FilteredAttendance = childAttendance
          .where((element) =>
              (Global.getItemValues(element.responces!, 'hosuehold_head_name'))
                  .toLowerCase()
  ...
  showDeleteDialog(ChildAttendanceResponceModel record) {
    showDialog(
        context: context,
        builder: (context) {
          return CustomDoubleButton(
              ...
              onPositive: () async {
                await AttendanceResponnceHelper().deleteDraftRecords(record);
                await attendeceChildRecord();
