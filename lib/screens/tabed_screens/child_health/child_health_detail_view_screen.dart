...
class ChildHealthDetailViewScreen extends StatefulWidget {
  final int enName;
  final int type;
  ...
  @override
  _ChildHealthDetailViewScreenState createState() =>
...
class _ChildHealthDetailViewScreenState
      ...
      child: Scaffold(
          appBar: CustomChildAppbar(
            text: Global.returnTrLable(
                labelControlls, CustomText.ChildHealth, lng!),
    ...
    if (items.length > 0) {
      for (int i = 0; i < items.length; i++) {
        ...
        if (element.reqd == 1) {
          ...
          //     setState(() {});
          //     Validate().singleButtonPopup(
          //         'A Visit Note already exists for the selected Date "${valuees}"',
          //         Global.returnTrLable(labelControlls, CustomText.ok, lng!),
          //         false,
          //         context);
          //     validStatus = false;
  ...
  Future<void> updateHiddenValue() async {
    ...
    if (alrecords.length > 0) {
      ...
      // if (alrecords[0].created_at != null) {
      //   var creationDate = DateTime.parse(alrecords[0].created_at!);
      //   if (creationDate
