...
import 'creche_monitering_checklist_SM_tab_forAdd.dart';

class AllcmcSMListingScreen extends StatefulWidget {
  // final String? creche_id;
   bool? isDraft;

   AllcmcSMListingScreen({
    super.key,
     this.isDraft
  });

  @override
  State<AllcmcSMListingScreen> createState() => _cmcSMListingScreenState();
}

class _cmcSMListingScreenState extends State<AllcmcSMListingScreen> {
  List<CmcSMResponseModel> cmcCCData = [];
  List<Translation> translats = [];
  String lng = 'en';
  String? selectedCreche;
  List<CmcSMResponseModel> filterData = [];
  GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  List<OptionsModel> creches = [];
  bool isOnlyUnsynched = false;
  List<CmcSMResponseModel> unsynchedList = [];
  List<CmcSMResponseModel> allList = [];
  ...
  Future<void> fetchCmcCCRecords() async {
    cmcCCData = await CmcSMTabResponseHelper().callCrechMonitoringSM();
    unsynchedList = cmcCCData
        .where((element) => element.is_edited == 1 || element.is_edited == 2)
        .toList();
    allList = cmcCCData;
    filterData = isOnlyUnsynched ? unsynchedList : allList; //change
    setState(() {});
  }
            ...
            onTap: () async {
              ...
              if (!(Global.validString(ccGuid))) {
                ...
                var refStatus = await Navigator.of(context).push(
                    MaterialPageRoute(
                        //change
                        builder: (BuildContext context) => CmcSMTabSCreenForAdd(
                            smguid: ccGuid,
                            isEdit: false,
                            isViewScreen: false)));
                    ...
                    ? ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return GestureDetector(
                            onTap: () async {
                              var ccGuid = filterData[index].smguid;
  ...
  showDeleteDialog(CmcSMResponseModel record) {
    showDialog(
        context: context,
        builder: (context) {
          return CustomDoubleButton(
              ...
              onPositive: () async {
                await CmcSMTabResponseHelper().deleteDraftRecords(record);
                await fetchCmcCCRecords();
