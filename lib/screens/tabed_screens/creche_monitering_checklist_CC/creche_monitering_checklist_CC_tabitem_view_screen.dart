...
import 'package:shishughar/model/dynamic_screen_model/creche_monitering_checklist_CC_response_model.dart';
...
class CmcCCTabItemViewSCreen extends StatefulWidget {
  ...
  @override
  State<CmcCCTabItemViewSCreen> createState() => _CmcCCTabItemViewSCreenState();
}

class _CmcCCTabItemViewSCreenState extends State<CmcCCTabItemViewSCreen> {
  ...
  Future<void> _initData() async {
    ...
    await _callScreenControllers('Creche Monitoring Checklist CC');
                          ...
                          child: CElevatedButton(
                            ...
                            onPressed: () {
                              if(widget.tabIndex == (widget.totalTab - 1))
                                Navigator.pop(context, 'itemRefresh');
                              else nextTab(1, context);
                              // widget.changeTab(1);
                            },
                            text:  Global.returnTrLable(
                                _translation, widget.tabIndex == (widget.totalTab - 1)?CustomText.exit:CustomText.Next, _language),
                          ),
                        ),
                      ],
                    ),
  ...
  Future<void> updateExistingFields() async {
    final records = await CmcCCTabResponseHelper()
    ...
    if (records.isNotEmpty) {
      ...
      if (crecheInfo.length > 0) {
        _myMap['appcreated_by'] = username;
        _myMap['appcreated_on'] = Validate().currentDateTime();
        _myMap['cmc_cc_guid'] = widget.cmc_cc_guid;
