...
import 'add_household_childred_form_expeded.dart';
...
class _ChildrenListingScreenState extends State<ChildrenListingScreen> {
  ...
  Future<void> fetchHhChildrenDataList() async {
    HouseHoldChildrenHelperHelper databaseHelper = HouseHoldChildrenHelperHelper();
    ...
    relationChilddata = await optionHelper.getMstCommonOptions('Relation',lng!);
      ...
      if (!checkChil) {
        ...
        if (alredRecord.length > 0) {
          if(alredRecord[0].is_edited==1) {
            ...
            await HouseHoldTabResponceHelper().updateResponce(
                jsonReso, widget.hhGuid);
          }
        }
      }
    }
                  ...
                  MaterialPageRoute(
                      builder: (BuildContext context) =>
                          AddHouseholdScreenChildrenFromExpended(
                            hhGuid: widget.hhGuid, cHHGuid: cHHGUID,dobisReadable:false)));
              ...
              return
                GestureDetector(
                  onTap: () async {
                    ...
                    var creationDate = DateTime.parse(hhChilddata[index].created_at.toString());
                    bool  dobisReadable = false;
                    if (creationDate.add(Duration(days: 15))
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Text(
                                callChildWithRelationText(Global.returnTrLable(
                                    childListingControlls,
                          ...
                          SizedBox(width: 5),
                          (hhChilddata[index].is_edited==0 && hhChilddata[index].is_uploaded==1)?
                          Image.asset(
                            "assets/sync.png",
                            scale: 1.5,
                          ):
  ...
  String getfindRelationValues(String id){
    ...
    if(Global.validString(id)){
      var reltionvl = relationChilddata.where((element) => element.name == id).toList();
  ...
  Future<bool> checkChildrenLimit(int totalChildren) async {
    ...
    if(children__3_years>totalChildren){return true;}else return false;

  }

  Future<void> setLabelText()async {
    lng = await Validate().readString(Validate.sLanguage);
    List<String> valueNames = ['Child Name','Relationship with child','Child Age (In Months)','Can not add children more than',CustomText.ok];

    await await TranslationDataHelper()
